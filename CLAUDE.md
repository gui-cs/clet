...
- **Bar-raise backlog** (label `bar-raise`) — critique that's been raised, considered, and *not yet* acted on. Items are tracked as individual issues: [#39](https://github.com/gui-cs/clet/issues/39) (Linux cold-start), [#40](https://github.com/gui-cs/clet/issues/40) (input-size caps), [#41](https://github.com/gui-cs/clet/issues/41) (keyboard nav), [#42](https://github.com/gui-cs/clet/issues/42) (two-repo ownership), [#37](https://github.com/gui-cs/clet/issues/37) (threat model). Before claiming a section is "done," check for open `bar-raise` issues in that area. New design pushback goes in a new `bar-raise`-labeled issue.
...
- **Did the PR resolve or invalidate a bar-raise item?** → Close or update the relevant `bar-raise`-labeled issue (see links above).
